...
class API:
    def __init__(self):
        self.__VersionDictionary = {'FwMusic_Version': 'v2.0.3',
                                    'API': 'v20213',
        ...
        for i in range(len(download_list_new)):
            if 1:
                if download_list[i][4]:  # Download vip
                    url = ncmApiManager.GetApi(self.api_type) + str(download_list[i][1]) + '&level=' + level
                    filename = download_list[i][0]
                    filename = simpleapi.format(filename)
                    ...
                    '''
                    elif self.api_type == 'Littleyouzi':
                        connection_pool = urllib3.PoolManager()
                        resp_url = connection_pool.request('GET', url)
                        # print(eval(resp_url.data.decode('utf-8'))['retcode'] == 0)
                        if eval(resp_url.data.decode('utf-8'))['retcode'] == 0:
                            self.url_true = str(eval(resp_url.data.decode('utf-8'))['data']['url'])
                        else:
                            flog.failure('Api[L] Error. Code:' + str(eval(resp_url.data.decode('utf-8'))['retcode']))
                            raise Exception
                        # print(self.url)
...
class Playlist_v2:
    ...
    def get_details(self):
        ...
        try:
            page_url = ncmApiManager.api_list_fast() + '/playlist/track/all?id=' + str(playlist_id)
            page_part_url = ncmApiManager.api_list_fast() + '/playlist/detail?id=' + str(playlist_id)
            Res = requests.request('GET', page_url).text
            Res = Res.replace('true', 'True')
            Res = Res.replace('false', 'False')
            Res = Res.replace('null', "'null'")
