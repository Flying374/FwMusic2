...
class API:
    def __init__(self):
        self.__VersionDictionary = {'FwMusic_Version': 'v2.0.11',
                                    'API': 'v20321',
...
config = Config('FwApi')
config.load()
config.write('Lyric_Encoding', 'ANSI')
config.save()
...
class Lyric:
    ...
    def get_lyric(self, id, name, artist):
        try:
            ...
            file.write(self.lyric.encode(config.read('Lyric_Encoding')))
            file.close()
        except Exception:
            Errsys.record('ErrL1')
