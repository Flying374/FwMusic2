...
class API:
    def __init__(self):
        self.__VersionDictionary = {'FwMusic_Version': 'v2.1.0W',
                                    'API': 'v20320',
...
class Config:
    def __init__(self):
        self.config_path = os.path.join('Configs', 'Configs.txt')
        self.config_dict = {}
...
class Downloader_v3_Auto:  # auto
    def __init__(self):
        ...
        self.url_true = ''  # Temp
        self.do_lyric = False
        self.current_download = '...'  # For 2.1.0W
        self.download_total = 0  # For 2.1.0W
    ...
    def download(self, download_list, artist_name, quality='standard', do_lyric=False):
        ...
        if quality == 'standard':
            ...
            for i in range(len(download_list)):
                if not download_list[i][4]:
                    try:
                        self.current_download = str(download_list[i][0])
                        url = 'http://music.163.com/song/media/outer/url?id=' + str(download_list[i][1])
                        # print(url)
                        filename = download_list[i][0]
                        filename = simpleapi.format(filename)
