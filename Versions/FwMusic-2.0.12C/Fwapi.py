...
class API:
    def __init__(self):
        self.__VersionDictionary = {'FwMusic_Version': 'v2.0.12C',
                                    'API': 'v20322',
...
class FLog:
    ...
    def debug(self, msg):
        ...
        log.write('[DEBUG] (' + str(time.strftime('%Y-%m-%d %H:%M:%S')) +
                  ')' + str(msg) + '\n')
    def exit(self):
        log = open(self.path, 'a', encoding='utf-8')
        log.write('[INFO] (' + str(time.strftime('%Y-%m-%d %H:%M:%S')) +
                  ') Exit FwMusic.' + '\n')
        log.close()
...
class ErrSystem:
    def __init__(self):
        ...
        self.__ErrDictionary = {'ErrE1': 'Error type is not in list.',
                                ...
                                'ErrC1': 'Can not read config file.'
                                }
...
class Config:
    def __init__(self, name='name'):
        self.name = name
        self.config_path = os.path.join('Config', name+'.txt')
        self.config_dict = {}
    ...
    def read(self, key, default=''):
        if key in self.config_dict:
            ...
            self.config_dict[key] = default
            with open(self.config_path, 'w', encoding='utf-8') as f:
                f.write(str(self.config_dict))
                f.close()
    ...
    def save(self):
        with open(self.config_path, 'w', encoding='utf-8') as f:
            f.write(str(self.config_dict))
            f.close()
        flog.info('Saving configs.name:'+ self.name)

Fwapi_config = Config('Fwapi')
Fwapi_config.load()
#config = Config('Optional')
#config.load()
#config.write('Temporary_Api', ' ')
#config.save()
#print(config.read('Lyric_Encoding'))
...
class NcmApiManager:
    def __init__(self):
        self.pre_api_list = ['https://zm.armoe.cn',#...
                              'https://163api.qijieya.cn',#...
                              'https://apis.netstart.cn/music',# CloudFlare!!!
                              'https://www.musicapi.cn',
                             'https://ncmapi.btwoa.com',
                             'https://api-music.kingcola-icg.cn',
    ...
    def Check_Api(self):
        flog.info('NcmApiManager : Checking Api.')
        opt = Config('Optional')
        opt.load()
        temp = self.pre_api_list
        temp.append(str(opt.read('Temporary_Api')))
        for test in temp:
            try:
                url = test + '/song/detail?ids=2041814115'
                #print(url, end='\n')
                ...
                if a['songs'][0]['name'] == '走过漫漫时空':
                    flog.info('Api:' + test + 'is valid.')
                    self.api_list_fast.append(test)
                else:
                    flog.failure('Api:' + test + ' is NOT valid.')
            except Exception:
                flog.failure('Api:' + test + ' is NOT valid.')
                flog.info('Total valid api amounts: ' + str(len(self.api_list_fast)))
        for apis in self.api_list_fast:
            flog.info('Api: ' + apis + ' is valid.')
        flog.info('NcmApiManager : Checking Api finished.')



ncmApiManager = NcmApiManager()
if Fwapi_config.read('SkipNcmApiCheck', default='False') != 'True':
    ncmApiManager.Check_Api()
...
class Lyric:
    ...
    def get_lyric(self, id, name, artist):
        try:
            ...
            lrc = Config('Configs')
            file.write(self.lyric.encode(lrc.read('Lyric_Encoding', default='utf-8')))
            file.close()
        except Exception:
            Errsys.record('ErrL1')
            ...
            for i in range(len(download_list_new)):
                try:
                    if download_list[i][4] or quality != 'standard':  # Download vip or high_quality
                        ...
                        if self.api_type == 'Byfuns':
                        ...
                        elif self.api_type == 'Toubiec':
                            # Test only !!!
                            quality = 'sky'
                            id = 1885205681
                            toubiec_dict = {'jymaster': '超清母带(Master)', 'sky': '沉浸环绕声(Surround Audio)',
                                            'jyeffect': '杜比全景声(Dolby Atmos)', }
                            quality_toubiec = toubiec_dict[quality]
                            ...
                            with sync_playwright() as playwright:
                                ...
                                page.goto("https://wyapi.toubiec.cn/")

                                input_locator = page.get_by_placeholder("请输入网易云音乐分享链接")  # 输入框定位器
                                input_locator.fill('https://music.163.com/#/song?id=' + str(id))

                                page.click("#quality-select .el-select__wrapper")
                                page.wait_for_selector(".el-select__popper", state="visible")
                                page.evaluate("""() => {
                                                                                                  const option = [...document.querySelectorAll('li')].find(el => el.textContent.includes('""" + quality_toubiec + """'));
                                                                                                  option.click();
                                                                                                }""")

                                submit_btn = page.query_selector('//*[@id="parse-btn"]')
                                submit_btn.click()
                                time.sleep(10)
                                context.close()
                                browser.close()
...
# The apis below are for FwMusic, not for FwApi.


class Flog_For_FwMusic:
